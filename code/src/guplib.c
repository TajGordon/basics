...
/* MATH STUFF */
/**************/
Vector2 Vec2AddV(Vector2 v1, Vector2 v2)
{
    return (Vector2){v1.x + v2.x, v1.y + v2.y};
}
Vector2 Vec2SubV(Vector2 v1, Vector2 v2)
{
    return (Vector2){v1.x - v2.x, v1.y - v2.y};
}
Vector2 Vec2MultV(Vector2 v1, Vector2 v2)
{
    return (Vector2){v1.x * v2.x, v1.y * v2.y};
}
Vector2 Vec2SubF(Vector2 v, float f)
{
    return (Vector2){v.x - f, v.y - f};
}
Vector2 Vec2MultF(Vector2 v, float f)
{
    return (Vector2){v.x * f, v.y * f};
}
Vector2 Vec2DivF(Vector2 v, float f)
{
    return (Vector2){v.x / f, v.y / f};
}
Vector2 Vec2AddF(Vector2 v, float f)
{
    return (Vector2){v.x + f, v.y + f};
}

Color ColorMultF(Color c, float f)
{
...
bool TileOnScreen(Vector2 tilepos, Camera2D camera)
{
    ...
    Vector2 tileMin = Vec2MultF(tilepos, TMSIZE);
    Vector2 tileMax = Vec2AddF(tileMin, TMSIZE);
...
{
    if (!foundSpecialItem && TileOnScreen(Vec2DivF(specialItemPos,TMSIZE), camera))
    {
...
void RenderShip(Camera2D camera, Player* p)
{
    ...
    if (TileOnScreen(Vec2SubV(Vec2DivF(shipPosition,TMSIZE), (Vector2){3, 0}), camera) || TileOnScreen(Vec2AddV(Vec2DivF(shipPosition,TMSIZE), (Vector2){3, 0}), camera)
     || TileOnScreen(Vec2SubV(Vec2DivF(shipPosition,TMSIZE), (Vector2){0, 3}), camera) || TileOnScreen(Vec2AddV(Vec2DivF(shipPosition,TMSIZE), (Vector2){0, 3}), camera))
    {
    ...
    if (shipOpen && Vector2DistanceSqr(p->pos, shipPosition) < (SHIPINTERACTIONDISTANCE * SHIPINTERACTIONDISTANCE))
    {
        ...
        {
            AddDisplayMessage(Vec2SubV(shipPosition, (Vector2){0, TMSIZE * 3}), 3,  "I can't leave without my special item\0");
    ...
    else if (Vector2DistanceSqr(p->pos, Vec2AddV(shipPosition, (Vector2){TMSIZE, 0})) < (SHIPINTERACTIONDISTANCE * SHIPINTERACTIONDISTANCE))
    {
...
void DrawBatteries(Player* p, Camera2D camera, double time)
{
    ...
    for (int i = bigjump; i < batterycount; i++)
    {
        ...
        {
            if (TileOnScreen(Vec2DivF(batteryPositions[i] , TMSIZE), camera))
            {
                DrawTextureV(batteryTexture, Vec2SubV(batteryPositions[i] , (Vector2){5, 3}), batteryColor[i]);
...
void LoadDoors()
{
    ...
    for (int i = 0; i < doorloadcount; i++)
    {
        ...
        doorEndPos[i] = (Vector2){doorStartPos[i].x, doorStartPos[i].y - numSpaces};

        // get key pos
        Vector2 totheleft = Vec2AddV(doorStartPos[i] , (Vector2){-1, 0});
        Vector2 totheright = Vec2AddV(doorStartPos[i] , (Vector2){1, 0});
...
void RenderDoors(Camera2D camera, Player* p)
{
    ...
    for (int i = 0; i < doorCount; i++)
    {
        ...
        if (TileOnScreen(doorStartPos[i], camera)) {
            ...
            DrawLineEx(Vec2AddV(Vec2MultF(doorStartPos[i] , TMSIZE) , (Vector2){DOORTHICKNESS/2., TMSIZE}), Vec2AddV(Vec2MultF(doorEndPos[i] , TMSIZE) , (Vector2){DOORTHICKNESS/2., 0}), DOORTHICKNESS, doorColor);
        }
        // keyhole
        if (TileOnScreen(doorKeyPos[i], camera))
        {
            ...
            { DrawTextureV(batteryTexture, Vec2MultF(doorKeyPos[i] , TMSIZE), batteryColor[doorKeyholebattery[i]]); }

            if (Vector2DistanceSqr(Vec2MultF(doorKeyPos[i] , TMSIZE), p->pos) < (INTERACTION_DISTANCE * INTERACTION_DISTANCE) && !didSomething)
            {
...
{
    return (Rectangle){p.pos.x - p.size.x/2, p.pos.y - p.size.y/2, p.size.x, p.size.y};
}
...
void ShootEnemyBullet(Bullet bullettype, Vector2 fromPosition, Vector2 direction)
{
    ...
    bulletsVs[bulletCount] = Vec2MultF(direction , bulletSpeed[bullettype]);
    bulletCount++;
}
...
void RenderRangedEnemies(Camera2D camera)
{
    ...
    {
        if (rangedEnemyAlive[i] && TileOnScreen(Vec2DivF(rangedEnemyPositions[i],TMSIZE), camera))
        {
...
void RangedEnemiesPhysicsProcess(Player* p)
{
    for (int i = 0; i < rangedEnemyCount; i++)
    {
        if (rangedEnemyAlive[i])
        {
            ...
            {
                ShootEnemyBullet(enemylightbullet, rangedEnemyPositions[i], Vector2Normalize(Vec2SubV(p->pos , rangedEnemyPositions[i])));
    ...
    for (int i = 0; i < enemyCount; i++)
    {
        ...
        if (!enemyDead[i])
        {
            ...
            if (enemyHealth[i] < 1)
            {
                ...
                AddDisplayMessage(Vec2SubV(enemyPositions[i] , (Vector2){0, TMSIZE}), 1, TextFormat("+%d", enemyScore[enemyTypes[i]]));
                aliveEnemyCount--;
                continue;
            }

            // Apply gravity if enemy is in the air
            if (!EnemyCollidingAt(i, Vec2AddV(enemyPositions[i] , (Vector2){0, 1}), solids, solidCount))
            {
            ...
            if (EnemyCollidingAt(i, Vec2AddV(enemyPositions[i] , enemyVelocities[i]), solids, solidCount))
            {
                enemyVelocities[i].x *= -1; // flip around
            }
            else
            { enemyPositions[i] = Vec2AddV(enemyPositions[i] , enemyVelocities[i]); }

            // Push players out of the way

            // Check if colliding with the player, if so, push the player
            AABB enemyAABB = {.max = Vec2AddV(enemyPositions[i] , Vec2DivF(enemySize[enemyTypes[i]] , 2)), .min = Vec2SubV(enemyPositions[i] , Vec2DivF(enemySize[enemyTypes[i]] , 2))};
            AABB playerAABB = {.max = Vec2AddV(p->pos , Vec2DivF(p->size , 2)), .min = Vec2SubV(p->pos , Vec2DivF(p->size , 2))};
            if (AABBsColliding(enemyAABB, playerAABB))
            {
                enemyDirections[i] *= -1;
                Vector2 pushDir = Vector2Normalize(Vec2SubV(p->pos , enemyPositions[i]));
                p->damageimpulse = Vec2MultF(pushDir , 5);// (Vector2){ENEMYHORIZONTALKNOCKBACKFORCE, 1.5}; // Adjust the push strength as needed
        ...
        else if (enemyDead[i]) // If the enemy is dead
        {
            ...
            if (enemySpawnTimers[i] < 0 + EPSILON)
            {
                ...
                enemyPositions[i] = Vec2MultF(enemySpawnPoints[i] , TMSIZE);



                enemyVelocities[i] = (Vector2){(float)((rand() % 2) ? 1 : -1) * ((rand() % 3) + 1), 0};
                enemyDead[i] = false;
                aliveEnemyCount++;
            }
        }
...
void RenderEnemies(Camera2D camera)
{
    for (int i = 0; i < enemyCount; i++)
    {
        if (!enemyDead[i] && TileOnScreen(Vec2DivF(enemyPositions[i],TMSIZE), camera))
        {
    ...
    for (int y = 0; y < WORLD_SIZE_Y; y++)
    {
        for (int x = 0; x < WORLD_SIZE_X; x++)
        {
            ...
            switch (c)
            {
                case 's':
                {
                    player_spawnpoint = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
                case 'S':
                {
                    shipPosition = Vec2AddF(Vec2MultF((Vector2){(float)x, (float)y} , TMSIZE) , (int)TMSIZE/2);
                ...
                case 'r':
                {
                    ...
                    rangedEnemyPositions[rangedEnemyCount] = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    rangedEnemyCount++;
                    break;
                }
                case 'P':
                ...
                case '*':
                {
                    specialItemPos = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
                case 'B':
                {
                ...
                case 'e':
                {
                    ...
                    enemySpawnPoints[enemyCount++] = (Vector2){(float)x, (float)y};
                    break;
                }
                case 'E':
                {
                ...
                case '0': // bigjump
                {
                    batteryPositions[0] = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
                case '1': // doublejump
                {
                    batteryPositions[1] = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
                case '2': // rapidfire
                {
                    batteryPositions[2] = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
                case '3': // heavyfire
                {
                    batteryPositions[3] = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
                case '4': // quickie
                {
                    batteryPositions[4] = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
                case '5': // tanky
                {
                    batteryPositions[5] = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
                case '6':
                {
                    batteryPositions[6] = (Vector2){(float)x * TMSIZE, (float)y * TMSIZE};
                    break;
                }
            }
            tilemap[x][y] = t;
...
void SpawnBullet(Bullet bulletType, float posx, float posy, float velx, float vely)
{
    ...
    bulletsTs[bulletCount] = bulletType;
    bulletsPs[bulletCount] = (Vector2){posx, posy};
    bulletsVs[bulletCount] = (Vector2){velx, vely};
    bulletCount++;
}
...
void BulletPhysicsProcess(Player* p, double time)
{
    ...
    for (int i = 0; i < bulletCount; i++)
    {
        bulletsPs[i] = Vec2AddV(bulletsPs[i] , Vec2MultF(bulletsVs[i] , physicsDTs));
    }
    ...
    // Check bullets collision
    {
        for (int i = 0; i < bulletCount; i++)
        {
            ...
            if (bulletsTs[i] == enemylightbullet && CheckCollisionCircleRec(bulletsPs[i], bulletRadius[bulletsTs[i]], GetPlayerRect(*p)))
            {
                ...
                p->damageimpulse = Vec2MultF(Vector2Normalize(bulletsVs[i]) , 5);
...
{
    AABB aabb = {.max = Vec2AddV(pos , Vec2DivF(a->size,2)), .min = Vec2SubV(pos , Vec2DivF(a->size,2))};
    // Check if colliding with the tilemap
    ...
    for (int j = 0; j < doorCount; j++)
    {
        if (!doorOpen[j])
        {
            AABB daabb = {.min = Vec2MultF(doorEndPos[j] , TMSIZE), .max = Vec2AddV(Vec2MultF(doorStartPos[j] , TMSIZE) , (Vector2){DOORTHICKNESS, TMSIZE})};
...
void MoveX(Player* a, float amount, Solid* solids, int solidCount)
{
    ...
    if (move != 0)
    {
        ...
        while (move != 0)
        {
            if (!PlayerCollidingAt(a, (Vector2){a->pos.x + sign, a->pos.y}, solids, solidCount))
            {
                a->pos.x += sign;
                move -= sign;
            }
...
void MoveY(Player* a, float amount, Solid* solids, int solidCount)
{
    ...
    if (move != 0)
    {
        ...
        while (move != 0)
        {
            if (!PlayerCollidingAt(a, (Vector2){a->pos.x, a->pos.y + sign}, solids, solidCount))
            {
                a->pos.y += sign;
                move -= sign;
            }
...
void PlayerInput(Player* p, double time)
{
    ...
    if (IsMouseButtonDown(MOUSE_LEFT_BUTTON) && time - p->lastShot > p->shootDelay)
    {
        ...
        SpawnBulletV(p->bullettype, p->pos, Vec2MultF(dir , bulletSpeed[p->bullettype]));
        p->lastShot = time;
    }
    ...
    if (dir.x != 0) p->lastDir = (Vector2){dir.x, 0};
    p->dir = dir;
...
void PlayerPhysicsProcess(Player* p, Solid* solids, int solidCount, double time)
{
    ...
    // Still on Ground check
    {
        ...
        if (PlayerCollidingAt(p, (Vector2){p->pos.x, p->pos.y + 1}, solids, solidCount))
        {
            p->grounded = true;
            p->persistentVel = (Vector2){0};
    ...
    // Impulse logic (not actual impulse idk what that is)
    {
        ...
        p->damageimpulse = Vec2SubV(p->damageimpulse , Vec2MultF(p->damageimpulse , physicsDTs));
...
void DrawSolids(Solid* solids, int solidCount)
{
    ...
    {
        Solid* s = &solids[i];
        DrawRectangleV(Vec2SubV(s->pos , Vec2DivF(s->size,2)), s->size, s->color);
    }
}
...
Solid MakeSolid(float x, float y, float width, float height, Color color, bool collideable)
{
    ...
    s.aabb.max = Vec2AddV(s.pos , Vec2DivF(s.size,2));
    s.aabb.min = Vec2SubV(s.pos , Vec2DivF(s.size,2));
    s.color = color;
...
void RenderDoorsWithLighting(Camera2D camera, Player* p)
{
    ...
    for (int i = 0; i < doorCount; i++)
    {
        ...
        if (visibilityGrid[gridX][gridY] != NOT_VISIBLE && TileOnScreen(doorStartPos[i], camera)) {
            ...
            DrawLineEx(Vec2AddV(Vec2MultF(doorStartPos[i] , TMSIZE) , (Vector2){DOORTHICKNESS/2., TMSIZE}),
                       Vec2AddV(Vec2MultF(doorEndPos[i] , TMSIZE) , (Vector2){DOORTHICKNESS/2., 0}),
        ...
        if (TileOnScreen(doorKeyPos[i], camera))
        {
            ...
            if (visibilityGrid[keyX][keyY] != NOT_VISIBLE)
            {
                ...
                if (doorKeyholeHasBattery[i])
                {
                    DrawTextureV(batteryTexture, Vec2MultF(doorKeyPos[i] , TMSIZE), ColorAlpha(batteryColor[doorKeyholebattery[i]], keyholeTint.a / 255.0f));
                }
...
void RenderBatteriesWithLighting(Player* p, Camera2D camera, double time)
{
    for (int i = bigjump; i < batterycount; i++)
    {
        if (!batteryInUse[i] && !batteryInDoor[i] && !batteryInShip[i])
        {
            ...
            if (visibilityGrid[gridX][gridY] != NOT_VISIBLE && TileOnScreen(Vec2DivF(batteryPositions[i] , TMSIZE), camera))
            {
                Color tint = (visibilityGrid[gridX][gridY] == BEHIND_WALL) ?
                    (Color){(unsigned char)(depthfactor * 255), (unsigned char)(depthfactor * 255), (unsigned char)(depthfactor * 255), 255} : WHITE;
                DrawTextureV(batteryTexture, Vec2SubV(batteryPositions[i] , (Vector2){5, 3}), ColorAlpha(batteryColor[i], tint.a / 255.0f));
            }
...
{
    {
        ...
        for (int i = 0; i < MAX_DISPLAY_MESSAGES; i++)
        {
            ...
            displayMessagesPositions[i] = (Vector2){};
            memset(displayMessages[i], 0, MAX_DISPLAYMESSAGE_SIZE);
        }
        for (int i = 0; i < MAX_ENEMIES; i++)
        {
            enemySpawnTimers[i] = 0;
            enemySpawnPoints[i] = (Vector2){};
            enemyPositions[i] = (Vector2){};
            enemyDirections[i] = 0;
            enemyHealth[i] = 0;
            enemyDead[i] = true;
        ...
        {
            doorStartPos[i] = (Vector2){};
            doorEndPos[i] = (Vector2){};
            doorOpen[i] = false;
            doorKeyPos[i] = (Vector2){};
...

    {
        ...
        p.FOV = DEFAULT_FOV_RADIUS;
        p.size = (Vector2){9, 14};
        p.lastDir = (Vector2){1, 0};
        p.col = (Color){0xff, 0xff, 0x00, 50};
        p.aabb.max = Vec2AddV(p.pos , Vec2DivF(p.size,2));
        p.aabb.min = Vec2SubV(p.pos , Vec2DivF(p.size,2));
        p.speed = NORMALSPEED;
        p.jumpVel = NORMALJUMPVEL;
        p.persistentVel = (Vector2){};
...

    {
        camera.zoom = 4;
        camera.rotation = 0;
        camera.offset = (Vector2){windowWidth / 2.f, windowHeight / 2.f + 90};
...

    {
        ...
        specialCamera.offset = (Vector2){windowWidth / 2.f, windowHeight / 2.f};
